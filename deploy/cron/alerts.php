...
while ($newlc =  mysql_fetch_assoc($query)) {
  $query2 = sprintf("SELECT user_id,distance,SQRT((69.1 * (%s - latitude) * 69.1 * (%s - latitude)) + (53 * (%s - longitude) * 53 * (%s - longitude))) AS lcdistance FROM searches WHERE saved='1' AND active='1' AND category_id='%s' HAVING lcdistance<distance",
